...
class TrackDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index):
        ...
        try:
            ...
            painter.drawRoundedRect(rect.adjusted(2, 2, -2, -2), _BORDER_RADIUS, _BORDER_RADIUS)
            ...
            # Index or Play Icon
            index_rect = QRectF(current_x, rect.top() + margin_y, 22, _CARD_HEIGHT - 2 * margin_y)
            if is_playing:
            ...
            # Cover
            cover_rect = QRectF(current_x, rect.top() + (_CARD_HEIGHT - _COVER_SIZE) / 2, _COVER_SIZE, _COVER_SIZE)
            ...
            if is_hovered:
                ...
                painter.drawRoundedRect(cover_rect, _COVER_SIZE // 4, _COVER_SIZE // 4)
                icon_rect = QRectF(cover_rect.center().x() - 11, cover_rect.center().y() - 11, 22, 22)
            ...
            text_rect = QRectF(current_x, rect.top() + margin_y, text_width, _CARD_HEIGHT - 2 * margin_y)
            
            title_rect = QRectF(text_rect.left(), text_rect.top() + 4, text_rect.width(), 20)
            author_rect = QRectF(text_rect.left(), text_rect.top() + 24, text_rect.width(), 20)
            ...
            elided_author = fm_auth.elidedText(author_str, Qt.ElideRight, text_rect.width())
            ...
            if is_hovered:
                # Draw Download btn
                dl_rect = QRectF(rect.right() - margin_x - 28, rect.top() + (_CARD_HEIGHT - 28) / 2, 28, 28)
                painter.setBrush(QColor(255, 255, 255, 15))
                painter.drawRoundedRect(dl_rect, 14, 14)
                self._dl_icon.paint(painter, dl_rect.adjusted(6, 6, -6, -6).toRect(), Qt.AlignCenter)
            ...
            source_h = 22
            source_rect = QRectF(current_right - source_w, rect.top() + (_CARD_HEIGHT - source_h) / 2, source_w, source_h)
