...
class PlaylistPage(QWidget):
    ...
    async def _populate_track_cards(
        self, tracks: list[Track], allow_remove: bool
    ) -> None:
    ...
    async def _on_play(self, track) -> None:
        if self._playlist:
            try:
                idx = self._playlist.tracks.values.index(track)
                self._playlist.set_current_track(idx)
    ...
    async def _play_all(self) -> None:
        if self._playlist and self._playlist.tracks.values:
            self._playlist.set_current_track(0)
    ...
    def _sync_playing_state(self, current_track=None) -> None:
        ...
        track = (
            current_track if current_track is not None else self._player.current_track
        )
        for card in self._cards:
