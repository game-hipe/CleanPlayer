...
class AsyncRecomendation:
    ...
    async def generate_radio_from_track(self, track: Track):
        ...
        with ThreadPoolExecutor() as thread:
            result = await asyncio.get_running_loop().run_in_executor(thread, lambda :self.client.get_watch_playlist(videoId=video_id, limit=10))
    ...
    async def get_id_if_not_yt(self, track: Track) -> str:
        result_id = await self.finder.get_tracks(track.title + " " + track.author, value=1)
